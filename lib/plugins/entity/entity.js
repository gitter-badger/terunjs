...
class Entity {
  ...
  async getEntitysConfig() {
    let getContentFile = filePath => _fs.default.readFileSync(`${this.baseDir}${this.configuration.entity_dir}/${filePath}`, 'utf-8');

    let contentToJson = content => content ? JSON.parse(content) : content;

    let result = this.files_to_work.map(getContentFile).map(contentToJson);
    return result;
  }
