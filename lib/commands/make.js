...
class Make {
  ...
  async createDir(to) {
    let dirToCreate = `${(0, _util.removeFileNameInPath)(to)}`;
  ...
  async getTransport(transport) {
    ...
    return new Promise(resolve => {
      ...
      _prompt.default.get(transport.args, async (err, result) => {
        ...
        let continueOverride = await this.continueOverrideFile(toFileName); // if(continueOverride)
  ...
  validInit(config, command) {
    let errorsBaseConfig = (0, _util.getMissingProperties)(config, ['commands']);
    let isValid = true;
