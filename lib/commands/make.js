...
class Make {
  ...
  async init() {
    ...
    await this.plugins.config(this.globalArgs); // get transport
  ...
  getTransport(transport) {
    return new Promise(resolve => {
      ...
      _prompt.default.get(transport.args, async (err, result) => {
        ...
        if (_fs.default.existsSync(toFileName)) {
          let continueTransport = await this.continue();

          if (continueTransport.length == 0) {
            console.log(_chalk.default.yellow('Relax, you skipped file, nothing to do :)'));
            return resolve();
          }
        }
