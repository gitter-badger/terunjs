...
class Make {
  constructor(config, command) {
    ...
    this.global_args = {};
    this.render = new _render.default(['>>', '<<']);
    this.plugins = {
  ...
  getTransport(transport) {
    return new Promise((resolve, reject) => {
      ...
      _prompt.default.get(transport.args, async (err, result) => {
        ...
        let to_file_rendered = this.render.renderSimple(to_file, argsToFileNameRender); // render final file
  ...
  async initPluginInArgsToRender(plugins, argsToParseViewRender = {}, argsToFileNameRender) {
    ...
    if (symfonyEntityReaderConfig) {
      ...
      symfonyEntityReaderPLUGIN.setConfig(symfonyEntityReaderConfig); // Colando parametros na raiz
